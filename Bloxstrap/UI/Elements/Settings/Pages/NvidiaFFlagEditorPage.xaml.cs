...
namespace Bloxstrap.UI.Elements.Settings.Pages
{
    ...
    {
        public ObservableCollection<NvidiaEditorEntry> Entries { get; } = new();

        private static readonly string NipDirectory = Path.Combine(Paths.Integrations, "Nvidia");
        private static readonly string NipPath = Path.Combine(NipDirectory, "Crystrap.nip");
        ...
        public ICollectionView EntriesView
        {
            ...
            {
                _entriesView = value;
                PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(EntriesView)));
            }
        }
        ...
        private void Add_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (Entries.Any(existing => existing.SettingId == entry.SettingId))
                    continue;
        ...
        private void DeleteSelected_Click(object sender, RoutedEventArgs e)
        {
            ...
            var toRemove = DataGridRef.SelectedItems.OfType<NvidiaEditorEntry>().ToList();
        ...
        private void DeleteAll_Click(object sender, RoutedEventArgs e)
        {
            ...
            var visibleEntries = EntriesView.Cast<NvidiaEditorEntry>().ToList();

            if (visibleEntries.Count == 0)
                return;

            if (Frontend.ShowMessageBox(
                    "Are you sure you want to delete all visible flags?",
                    MessageBoxImage.Warning,
                    MessageBoxButton.YesNo) != MessageBoxResult.Yes)
            {
                return;
        ...
        private void CopySelected_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                Clipboard.SetText(File.ReadAllText(NipPath, Encoding.Unicode));
                Frontend.ShowMessageBox("NIP file copied to clipboard.", MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                Frontend.ShowMessageBox($"Failed to copy NIP file:\n{ex.Message}", MessageBoxImage.Error);
            }
        }
        ...
        private void ExportNip_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                Frontend.ShowMessageBox("NVIDIA profile exported successfully.", MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
        ...
        private async void ResetNipFile_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Frontend.ShowMessageBox($"Error while resetting the NIP file: {ex.Message}");
            }
        }
        ...
        private void SetupFileWatcher()
        {
            ...
            _watcher = new FileSystemWatcher(NipDirectory, "Crystrap.nip")
            {
        ...
        {
            Directory.CreateDirectory(NipDirectory);
            File.WriteAllText(NipPath, NvidiaProfileManager.EmptyNipTemplate(), new UnicodeEncoding(false, true));
        }
    }
}
