...
namespace Bloxstrap.Utility
{
    static class WindowsRegistry
    {
        ...
        public static void RegisterProtocol(string key, string name, string handler, string handlerParam = "%1")
        {
            ...
            if (uriKey.GetValue("") as string != $"URL: {name} Protocol")
                uriKey.SetValueSafe("", $"URL: {name} Protocol");

            if (uriKey.GetValue("URL Protocol") as string != "")
        ...
        /// </summary>
        public static void RegisterPlayer() => RegisterPlayer(Paths.Application, "\"%1\"");
