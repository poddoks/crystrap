    ...
    internal class Installer
    {
        ...
        public void DoInstall()
        {
            ...
            if (ImportSettings)
            {
                ...
                // if something doenst exist crystrap will recreate the file/directory
                try
                {
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public static void SyncRuntimeFiles(string sourceDirectory, string destinationDirectory)
        {
            ...
            App.Logger.WriteLine(LOG_IDENT, $"Syncing runtime from {sourceDirectory} to {destinationDirectory}");
            ...
            string normalizedDestination = Path.GetFullPath(destinationDirectory).TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);

            if (String.Equals(normalizedSource, normalizedDestination, StringComparison.OrdinalIgnoreCase))
            {
                App.Logger.WriteLine(LOG_IDENT, "Source and destination are the same directory, skipping sync");
                return;
            }
            ...
            foreach (string sourcePath in Directory.GetFileSystemEntries(sourceDirectory))
            {
                ...
                if (RuntimeCopyExcludedNames.Contains(name))
                    continue;
        ...
        private static void CopyDirectory(string sourceDirectory, string destinationDirectory)
        {
            ...
            foreach (string file in Directory.GetFiles(sourceDirectory, "*", SearchOption.AllDirectories))
            {
                ...
                CopyFileWithLockedDestinationCheck(file, destinationFile);
            }
        }

        private static void CopyFileWithLockedDestinationCheck(string sourcePath, string destinationPath)
        {
            const string LOG_IDENT = "Installer::CopyFileWithLockedDestinationCheck";
        ...
        public void ImportSettingsFromBloxstrap()
        {
            ...
            foreach (string FileName in FilesForImporting)
            {
                ...
                if (IsDirectory)
                {
                    // delete existing file from crystrap folder
