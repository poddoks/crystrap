...
namespace Bloxstrap
{
    public class Bootstrapper
    {
        ...
        private bool HasRequiredAppFiles()
        {
        ...
        private bool IsInstalledAndHealthy()
        {
            if (!App.Settings.Prop.AutoRepairBrokenRobloxInstallBeforeLaunch)
                return File.Exists(AppData.ExecutablePath);

            return HasRequiredAppFiles();
        }

        private IEnumerable<(string RelativeDirectory, string Label, bool Enabled)> GetPerformanceStripTargets()
        {
            bool extremeStripMode = App.Settings.Prop.ExtremeStripMode;

            yield return (@"PlatformContent\pc\terrain", "terrain textures", App.Settings.Prop.DisableTerrainTextures || extremeStripMode);
        ...
        public async Task Run()
        {
            ...
            if (!_noConnection && App.Settings.Prop.CleanupStaleRobloxFilesBeforeLaunch)
            {
                var activePackageHashes = App.RobloxState.Prop.Player.PackageHashes.Values
                    .Concat(App.RobloxState.Prop.Studio.PackageHashes.Values)
                    .Concat(_versionPackageManifest.Select(x => x.Signature))
                    .Distinct(StringComparer.OrdinalIgnoreCase);

                Integrations.Cleaner.CleanupBeforeLaunch(_latestVersionGuid, activePackageHashes);
            }
        ...
        public async Task<bool> PreparePureLaunchAsync()
        {
            const string LOG_IDENT = "Bootstrapper::PreparePureLaunchAsync";
            ...
            _versionPackageManifest = new PackageManifest("v0\n");

            _versionPackageManifest.AddRange(AppData.State.PackageHashes.Select(x => new Package
            {
                Name = x.Key,
                Signature = x.Value
            }));
        ...
        private async Task StartRoblox()
        {
            ...
            if (_launchMode == LaunchMode.Player)
            {
                ...
                bool isPlayerProtocolLaunch = _launchCommandLine.StartsWith("roblox-player:", StringComparison.OrdinalIgnoreCase);
                ...
                {
                    if (isPlayerProtocolLaunch)
                    {
                        App.Logger.WriteLine(LOG_IDENT, "Skipping better matchmaking launch rewrite for roblox-player payload to preserve browser login handoff");
                    }
                    else
                    {
            ...
            if (_launchMode == LaunchMode.Player || App.Settings.Prop.EnableActivityTracking || App.LaunchSettings.TestModeFlag.Active || autoclosePids.Any())
            {
                ...
                var watcherData = new WatcherData
                {
                    ...
                    IsPlayer = _launchMode == LaunchMode.Player
                };
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            return await App.CheckForUpdatesAsync(_launchMode, Dialog);
        }
        #endregion
        ...
        private async Task<bool> SyncPerformanceStripTargetsAsync()
        {
            const string LOG_IDENT = "Bootstrapper::SyncPerformanceStripTargetsAsync";

            bool success = true;

            foreach (var target in GetPerformanceStripTargets())
            {
                string versionDirectory = Path.Combine(_latestVersionDirectory, target.RelativeDirectory);
                ...
                if (target.Enabled)
                {
                    try
                    {
                        ...
                        App.Logger.WriteLine(LOG_IDENT, $"Stripped {target.Label} from the current Roblox version");
                    }
                    catch (Exception ex)
                    {
                        App.Logger.WriteLine(LOG_IDENT, $"Failed to strip {target.Label}");
                        App.Logger.WriteException(LOG_IDENT, ex);
                        success = false;
                    }
                ...
                bool targetMissing = !Directory.Exists(versionDirectory) || !Directory.EnumerateFileSystemEntries(versionDirectory).Any();
                ...
                try
                {
                    await RestoreStrippedDirectoryAsync(target.RelativeDirectory, target.Label);
                }
                catch (Exception ex)
                {
        ...
        private async Task RestoreStrippedDirectoryAsync(string relativeDirectory, string label)
        {
            const string LOG_IDENT = "Bootstrapper::RestoreStrippedDirectoryAsync";

            var packages = GetPackagesForRelativeDirectory(relativeDirectory).ToList();

            if (!packages.Any())
            {
                App.Logger.WriteLine(LOG_IDENT, $"No package mapping was found for {relativeDirectory}, skipping restore");
                return;
            }
            ...
            App.Logger.WriteLine(LOG_IDENT, $"Restored {label} from package data");
        }

        private void RestoreStrippedDirectoriesFromPackages()
        {
            const string LOG_IDENT = "Bootstrapper::RestoreStrippedDirectoriesFromPackages";
            ...
            foreach (var target in GetPerformanceStripTargets())
            {
                ...
                foreach (var package in GetPackagesForRelativeDirectory(target.RelativeDirectory))
                {
                    if (!File.Exists(package.DownloadPath))
                    {
                        App.Logger.WriteLine(LOG_IDENT, $"Package {package.Name} is missing at {package.DownloadPath}, skipping restore for {target.RelativeDirectory}");
                        continue;
                    }
        ...
        private IEnumerable<Package> GetPackagesForRelativeDirectory(string relativeDirectory)
        {
            string normalizedDirectory = relativeDirectory.Replace('/', '\\').Trim('\\');

            return _versionPackageManifest.Where(package =>
            {
                string? packageDirectory = PackageDirectoryMap.GetValueOrDefault(package.Name);

                if (String.IsNullOrEmpty(packageDirectory))
                    return false;

                return String.Equals(packageDirectory.Replace('/', '\\').Trim('\\'), normalizedDirectory, StringComparison.OrdinalIgnoreCase);
            });
        }

        private void RestorePureClientState()
        {
            const string LOG_IDENT = "Bootstrapper::RestorePureClientState";
            ...
            foreach (string fileLocation in App.RobloxState.Prop.ModManifest)
            {
                ...
                if (String.IsNullOrEmpty(packageName))
                {
                    ...
                    App.Logger.WriteLine(LOG_IDENT, $"Deleting non-package modification {fileLocation}");
                ...
                fileRestoreMap[packageName].Add(fileName);
                App.Logger.WriteLine(LOG_IDENT, $"Restoring {fileLocation} from {packageName}");
            }
            ...
            foreach (var entry in fileRestoreMap)
            {
                ...
                if (!File.Exists(package.DownloadPath))
                {
                    App.Logger.WriteLine(LOG_IDENT, $"Package {package.Name} is missing at {package.DownloadPath}, skipping restore");
                    continue;
                }
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, "Removing ClientAppSettings.json for pure launch");
                File.Delete(clientSettingsPath);
            }

            RestoreStrippedDirectoriesFromPackages();
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex.GetType() == typeof(ChecksumFailedException))
                    {
                        Frontend.ShowConnectivityDialog(
                            ...
                            String.Format(Strings.Dialog_Connectivity_UnableToDownloadReason, "[https://github.com/poddoks/crystrap](https://github.com/poddoks/crystrap)"),
